 ...
 * 提供可配置的快捷键系统
 */
export class HotkeyManager extends Interaction implements IInteraction {
  name = 'hotkey-manager';
  ...
  // 默认快捷键配置
  private static DEFAULT_HOTKEYS: HotkeyConfig[] = [
   ...
   * 注销快捷键
   */
  unregisterHotkey(config: Omit<HotkeyConfig, 'action'>) {
    const key = this.buildHotkeyKey(config as HotkeyConfig);
   ...
   * 注册动作处理器
   */
  registerAction(action: HotkeyAction) {
   ...
   * 获取快捷键帮助信息
   */
  getHotkeyHelp(): Array<{ shortcut: string; description: string }> {
    const help: Array<{ shortcut: string; description: string }> = [];

    this.hotkeys.forEach((config) => {
      const shortcut = this.formatHotkey(config);
      const action = this.actions.get(config.action);
      const description =
        config.description || action?.description || config.action;
  ...
  private buildHotkeyKey(config: HotkeyConfig | Omit<HotkeyConfig, 'action'>): string {
    const parts: string[] = [];
  ...
  private formatHotkey(config: HotkeyConfig): string {
    const parts: string[] = [];
    const isMac =
      typeof navigator !== 'undefined' &&
      navigator.platform.toLowerCase().includes('mac');

    if (config.ctrl || config.meta) parts.push(isMac ? '⌘' : 'Ctrl');
  ...
  private onKeyDown = async (event: KeyboardEvent) => {
    if (!this.enabled || !this.interaction.isActive()) return;
