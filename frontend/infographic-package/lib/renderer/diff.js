...
exports.needsFullRerender = needsFullRerender;
exports.getUpdateSelectors = getUpdateSelectors;
/**
 * 比较两个数据对象，返回差异
 */
 ...
 * 比较数据项数组
 */
function diffItems(oldItems, newItems, parentIndexes = []) {
    const changes = [];
 ...
 * 比较单个数据项
 */
function diffSingleItem(oldItem, newItem, indexes) {
    const changes = [];
    const basePath = `items[${indexes.join('][')}]`;
    // 比较基本字段
    const fields = ['label', 'desc', 'value', 'icon', 'illus'];
    ...
    if (oldItem.children || newItem.children) {
        const childChanges = diffItems(oldItem.children || [], newItem.children || [], indexes);
 ...
 * 根据差异结果判断是否需要完整重渲染
 */
function needsFullRerender(diff) {
    return diff.type === 'full';
}
/**
 * 获取需要更新的元素选择器
 */
function getUpdateSelectors(changes) {
    const selectors = [];
