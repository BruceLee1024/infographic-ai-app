...
const HierarchyTree = (props) => {
    ...
    const renderNode = (node, levelBounds, btnBounds, offsets, gradientDefs, allNodes) => {
        ...
        // 计算节点颜色
        const colorIndexes = (0, utils_1.getHierarchyColorIndexes)({
            depth,
        ...
        if (parent) {
            ...
            if (edgeColorMode === 'gradient') {
                // 使用渐变色
                const parentColorIndexes = (0, utils_1.getHierarchyColorIndexes)({
                    depth: parent.depth,
                    originalIndexes: parent.data._originalIndex,
                    flatIndex: parent.data._flatIndex,
                }, colorMode);
                const childColorIndexes = (0, utils_1.getHierarchyColorIndexes)({
                    depth,
                    originalIndexes: indexes,
                    flatIndex: nodeData._flatIndex,
                }, colorMode);
